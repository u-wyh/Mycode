...
// 提交如下代码，可以通过所有测试用例
// 这道题第二次离线操作的查询必须要优化到O(1)  其实所有的第二次的查询都必须是O(1)  否则时间复杂度就会难以接受
// 但是第二次离线操作的修改操作时间复杂度可以使O(根号n)  因此我们第二次离线操作使用分块算法实现
// 因为只会移动n次  所以修改操作哪怕是O(根号n)也是可以接受的   可是查询操作是O(n*根号n)次的 所以查询只能是O(1)
#include <bits/stdc++.h>
using namespace std;
