...
// 这道题需要统计出现次数  因为回文自动机上都是本质不同的回文串
// 那么次数就是本质相同的回文串的出现次数  加上是别的回文串的后缀次数
// 本质相同的回文串次数好算  就是在每次添加一个字符后 出现的次数加一
// 如果一个回文串是另外一个的最长子回文后缀  那么他出现的次数应该要加上这个的出现次数
// 因为这个回文串一定是包含了这个最长的子回文后缀的
// 这里其实是一个类似于fail树的操作
#include<bits/stdc++.h>
using namespace std;
